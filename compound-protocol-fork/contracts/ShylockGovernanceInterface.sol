...
abstract contract ShylockGovernanceInterface {
    ...
    function getProtoclGuaranteeRate(address dao) virtual external returns (uint);
    function getMemberCap(address member) virtual external returns (uint);
    function getMemberGuaranteeRate(address member) virtual external returns (uint);
