...
contract ShylockComptroller is Comptroller, ShylockComptrollerInterface, ShylockComptrollerStorage{
    ...
    function getAccountReserve(address account) public view returns (Error, uint) {
        uint totalReserve;
        uint assetReserve;
        uint oraclePriceMantissa;
        Exp memory oraclePrice;
        ...
        return (Error.NO_ERROR, totalReserve);
    }

    function getAccountGuarantee(address account) public view returns (Error, uint) {
        uint totalGuarantee;
        uint assetGuarantee;
        uint oraclePriceMantissa;
        Exp memory oraclePrice;
        ...
        return (Error.NO_ERROR, totalGuarantee);
    }
    ...
    function getHypotheticalAccountReserveInternal(
        ...
        uint daoReserve;
        (err, daoReserve) = getAccountReserve(dao); // 60
        if (err != Error.NO_ERROR) {
            return (err, 0, 0);
        }
        uint memberReserve;
