...
window.onload = () => {
  ...
  const updateUrlBtn = document.querySelector("#updateUrlBtn");
  if (updateUrlBtn) {
    updateUrlBtn.addEventListener("click", updateHostUrl);
  }
};
...
function updateHostUrl() {
  const newHostUrl = document.querySelector("#newHostUrl").value;
  if (!newHostUrl) {
